...
public sealed partial class PerformanceViewModel : ObservableObject
{
    ...
    public PerformanceViewModel(ModuleOrchestrator orchestrator, BrokerClient broker)
    {
    ...
    {
        IsScanning = true;
        Status = "Analizando rendimiento…";
        _pendingChangeSet = null;

        try
        {
            var module = _orchestrator.Modules.FirstOrDefault(m => m.Metadata.Id == "wintune.performance");
            ...
            Status = $"{Findings.Count} problema(s) encontrado(s)";
        }
    ...
    private async Task ApplyAsync()
    {
        ...
        try
        {
            ...
            var module = _orchestrator.Modules.First(m => m.Metadata.Id == "wintune.performance");
