...
public sealed partial class GamingViewModel : ObservableObject
{
    ...
    public GamingViewModel(ModuleOrchestrator orchestrator, BrokerClient broker)
    {
    ...
    {
        IsScanning = true;
        Status = "Escaneando configuración gaming…";
        _pendingChangeSet = null;

        try
        {
            var module = _orchestrator.Modules.FirstOrDefault(m => m.Metadata.Id == "wintune.gaming");
            ...
            Status = $"{Findings.Count} elementos encontrados — {FormatBytes(EstimatedBytesSaved)} a liberar";
        }
    ...
    private async Task ApplyAsync()
    {
        ...
        try
        {
            ...
            Status = "Aplicando optimizaciones…";

            var module = _orchestrator.Modules.First(m => m.Metadata.Id == "wintune.gaming");
