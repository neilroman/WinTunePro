...
public sealed partial class CleanupViewModel : ObservableObject
{
    ...
    public CleanupViewModel(ModuleOrchestrator orchestrator, BrokerClient broker)
    {
    ...
    {
        IsScanning = true;
        Status = "Escaneando archivos temporales…";
        _pendingChangeSet = null;

        try
        {
            var module = _orchestrator.Modules.FirstOrDefault(m => m.Metadata.Id == "wintune.cleanup");
    ...
    private async Task ApplyAsync()
    {
        ...
        try
        {
            ...
            Status = "Eliminando archivos…";

            var module = _orchestrator.Modules.First(m => m.Metadata.Id == "wintune.cleanup");
