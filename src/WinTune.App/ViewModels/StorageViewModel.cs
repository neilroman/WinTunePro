...
public sealed partial class StorageViewModel : ObservableObject
{
    ...
    public StorageViewModel(ModuleOrchestrator orchestrator, BrokerClient broker)
    {
    ...
    {
        IsScanning = true;
        Status = "Analizando almacenamiento…";
        _pendingChangeSet = null;

        try
        {
            var module = _orchestrator.Modules.FirstOrDefault(m => m.Metadata.Id == "wintune.storage");
    ...
    private async Task ApplyAsync()
    {
        ...
        try
        {
            ...
            Status = "Liberando espacio…";

            var module = _orchestrator.Modules.First(m => m.Metadata.Id == "wintune.storage");
