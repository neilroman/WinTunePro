...
using WinTune.Plugins.Devtools;
using WinTune.Sdk;
using Xunit;

namespace WinTune.Plugins.Devtools.Tests;

public sealed class DevtoolsModuleTests
{
    private readonly DevtoolsModule _module = new(NullLogger<DevtoolsModule>.Instance);

    [Fact]
    public void Metadata_HasExpectedId()
    {
        _module.Metadata.Id.Should().Be("wintune.devtools");
        _module.Metadata.Category.Should().Be(ModuleCategory.Developer);
    }
    ...
    public async Task ScanAsync_ReturnsListOfFindings()
    {
        ...
        {
            f.ModuleId.Should().Be("wintune.devtools");
            f.Id.Should().NotBeNullOrEmpty();
        });
    }

    [Fact]
    public async Task PreviewAsync_BuildsOneDeleteChangePerPath()
    {
        ...
        try
        {
            var finding = Finding.Create(
                ...
                metadata: new Dictionary<string, object> { ["paths"] = new List<string> { tmp1, tmp2 }, ["tool"] = "npm" });

            // Act
            var cs = await _module.PreviewAsync([finding], CancellationToken.None);

            // Assert
            cs.ModuleId.Should().Be("wintune.devtools");
    ...
    public async Task ApplyAsync_DeletesTargetFiles()
    {
        ...
        var cs = new ChangeSet(Guid.NewGuid().ToString(), "wintune.devtools", [change], 0, DateTimeOffset.UtcNow);
    ...
    [Fact]
    public async Task ApplyAsync_HandlesNonExistentFile()
    {
    ...
    public async Task RevertAsync_CompletesWithoutThrowing()
    {
        var fakeResult = new ApplyResult(
            "cs1", "wintune.devtools", true, [], [], null, DateTimeOffset.UtcNow);
